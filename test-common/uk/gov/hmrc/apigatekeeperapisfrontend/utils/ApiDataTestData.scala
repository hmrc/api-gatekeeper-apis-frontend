...
import uk.gov.hmrc.apiplatform.modules.apis.domain.models.ApiDefinition.{Description, Name, ServiceBaseUrl}
import uk.gov.hmrc.apiplatform.modules.common.domain.models.{ApiContext, ApiVersionNbr, Environment}
...
trait ApiDataTestData extends FixedClock {
  ...
  val defaultEvent: DisplayApiEvent   = DisplayApiEvent(defaultServiceName, instant, "API Version Change", Environment.Sandbox, List.empty)

  val defaultApiDefinition: ApiDefinition = ApiDefinition(
    serviceName = defaultServiceName,
    serviceBaseUrl = ServiceBaseUrl("a.com/"),
    name = Name("Hello World"),
    description = Description("This is a test api"),
