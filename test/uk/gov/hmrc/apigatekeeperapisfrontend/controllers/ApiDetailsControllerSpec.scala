...
class ApiDetailsControllerSpec extends AsyncHmrcSpec with GuiceOneAppPerSuite {
  ...
  trait Setup extends MockitoSugar with ArgumentMatchersSugar with StrideAuthorisationServiceMockModule with LdapAuthorisationServiceMockModule with ApmServiceMockModule {
    val page        = app.injector.instanceOf[ApiDetailsPage]
    val error       = app.injector.instanceOf[ErrorTemplate]
    val mcc         = app.injector.instanceOf[MessagesControllerComponents]
    val serviceName = ServiceName("fish")

    val controller = new ApiDetailsController(mcc, StrideAuthorisationServiceMock.aMock, LdapAuthorisationServiceMock.aMock, page, error, ApmServiceMock.aMock)
    ApmServiceMock.returnsSingleApi(serviceName)
  }
  ...
  "GET /" should {
    ...
    "return Error Template" in new Setup {
      StrideAuthorisationServiceMock.Auth.succeeds(GatekeeperRoles.USER)
      ApmServiceMock.returnsNoSingleApi(serviceName)
      val result = controller.page(serviceName)(fakeRequest)
      contentType(result) shouldBe Some("text/html")
      charset(result) shouldBe Some("utf-8")
